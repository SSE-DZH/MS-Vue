  ...
  <div>
    <el-card>
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="schedule" label="时间" width="150"></el-table-column>
...
export default {
  methods: {
    deleteSCT(row) {
      ...
      const that = this
      axios.post('http://springboot_app:10086/SCT/deleteBySCT', sct).then(function (resp) {
        if (resp.data === true) {
          that.$message({
            showClose: true,
            message: '退课成功',
  ...
  created() {
    ...
    const term = sessionStorage.getItem('currentTerm')
    const that = this
    axios.get('http://springboot_app:10086/SCT/findBySid/' + sid + '/' + term).then(function (resp) {
